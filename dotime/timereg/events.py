...
class HandleEvents:
        ...
        try:
            ...
            with conn.cursor() as cur:
                stmt = sql.SQL("""
                    ...
                    DO NOTHING
                """).format(event_type=sql.Literal(event_type))
                cur.execute(stmt)
                conn.commit()
                added = True
    ...
    def get_event_type(cls, eventname):
        ...
        try:
            ...
            with conn.cursor() as cur:
                ...
                """).format(eventname=sql.Literal(eventname))
                cur.execute(stmt)
                if cur.rowcount:
    ...
    def toggle_event(self, event_type_uuid, the_date, user_id):
        ...
        if self.is_event_registered(event_type_uuid, the_date, user_id):
            current_app.logger.debug(
                "Event is registered. Deleting it.",
            )
    ...
    def get_commute_status(self, user_id, the_date):
        ...
        worked_from_home_status = self.is_event_registered(
            work_at_home_event_type, the_date, user_id
        )
        commuted_to_work_status = self.is_event_registered(
            commute_event_type, the_date, user_id
        )
