...
from dotime.timereg import register, events
...
def enter_time():
    # TODO: refactor and bring down number of local vars
    ...
    total_norm_hours_week = settings_obj.get_number_of_work_hours_for_a_week(
        session.get("user_id")
    )

    percentage_hours_worked_this_week = reg.percentage_worked(
        session.get("user_id"), time_date, "week"
    )
    percentage_hours_worked_this_day = reg.percentage_worked(
        session.get("user_id"), time_date, "day"
    )
    ...
    return render_template(
        ...
        weekday_length=f"{weekday_length_hour}:{weekday_length_minutes}",
        ...
        percentage_hours_worked_this_day=percentage_hours_worked_this_day,
    )
...
def register_time():
    ...
    if request.method == "POST":
        ...
        if time_reg.add_timeregistration(
            activity_uuid, time_date, time_start, time_end
        ):
...
def get_activity_by_name(activity_name):
    ...
    item_list = do_register.create_select2_data_structure_for_ajax_call(
        data, no_list="true"
    )
    return jsonify(item_list)
