        ...
        try:
            with conn.cursor() as cur:
                stmt = sql.SQL("""
                    ...
                    RETURNING usersid
                """).format(username=sql.Literal(username), email=sql.Literal(email))
                cur.execute(stmt)
                users_id = cur.fetchone()[0]
                if users_id:
                    password_added = self.add_user_password(
                        conn, users_id, hashed_password
                    )
                    if password_added:
    ...
    def update_profile(cls, users_id, email):
        ...
        try:
            ...
            with conn.cursor() as cur:
                ...
                """).format(email=sql.Literal(email), users_id=sql.Literal(users_id))
                cur.execute(stmt)
    ...
    def get_email_by_uuid(cls, user_id):
        ...
        try:
            ...
            current_app.logger.error(
                "Error performing sql query. SQL %s - error %s", sql, error
            )
        finally:
            conn.close()
        return email
