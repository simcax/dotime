...
def add_activity_registrations(
    user_id, number_of_activities, number_of_registrations, date_of_registrations=None
):
    """
...
def test_add_timereg_overlapping_registrations(create_user, app_test_context):
    ...
    with app_test_context:
        ...
        assert isinstance(activity_uuid, str)
        timereg_added_1 = time_reg.add_timeregistration(
...
def test_get_time_registered_this_week(create_user, app_test_context):
    ...
    with app_test_context:
        ...
        # Establish the start time and date for the reigstration
        start_timefrom = datetime.datetime.now()
        start_of_week, end_of_week = date_util.get_start_end_of_week(
            start_timefrom.strftime("%Y-%m-%d")
        )
        day_1 = start_of_week
        ...
        time_reg.add_timeregistration(
            activity_uuid, day_1, timefrom1, timeto1, testing=True
        )
        time_reg.add_timeregistration(
            activity_uuid, day_2, timefrom2, timeto2, testing=True
        )
...
def test_minutes_over_60_converted_to_hours_when_getting_time_for_week(
    create_user, app_test_context
):
    """
    ...
    with app_test_context:
        ...
        time_reg.add_timeregistration(
            activity_uuid, day_3, timefrom3, timeto3, testing=True
        )
...
def test_get_percentage_worked_of_intended_hours(create_user, app_test_context):
    ...
    with app_test_context:
        ...
        # Timestamps for 2nd entry - 1 hour worked on tuesday
        timefrom2_timestamp = timeto_timestamp + datetime.timedelta(days=1)
        timeto2_timestamp = timefrom2_timestamp + datetime.timedelta(
            hours=7, minutes=30
        )
        day_2 = timefrom2_timestamp.strftime("%Y-%m-%d")
        # Hour and minute times of 2nd entry
